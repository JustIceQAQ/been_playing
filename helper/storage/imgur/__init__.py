...
class ImgurStorage(StorageInit):
    ...
    def __init__(
        self, cache_file_path: Path | None = None, use_cache: bool | None = True
    ):
        ...
        self._cache_file_path = (
            cache_file_path
            or Path(__file__).resolve(strict=True).parent.parent.parent.parent
        )
        self._client = Client()
        self._temp_cache_data: dict | None = None
        self.use_cache = use_cache

    def _load_cache_data(self):
        with open(
            self._cache_file_path / Path("data/cache_file.json"), "a+", encoding="utf-8"
        ) as file:
            file.seek(0)
            try:
    ...
    def _dump_cache_data(self):
        with open(
            self._cache_file_path / Path("data/cache_file.json"), "w", encoding="utf-8"
        ) as file:
            json.dump(self._temp_cache_data, file, indent=4)
        del self._temp_cache_data
        self._temp_cache_data = None

    def _get_cache_url(self, image_url_hash: str) -> str | None:
    ...
    def upload(self, image_url: str | None) -> str | None:
        """
        ...
        with self._lock:
            ...
            if response.is_success:
                ...
                formatted_uri = self._uploaded_image_url.format(
