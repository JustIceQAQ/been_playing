...
class TWTCCrawler:
    def __init__(self, url: str):
        self.url = url
        self._view_state_generator: str | None = None
        self._event_validation: str | None = None
    ...
    def _is_first(self) -> bool:
        return bool(
            self._view_state_generator and self._event_validation and self._view_state
        )

    def _reload_payload(self, parsed_response: "BeautifulSoup"):
        view_state_generator = parsed_response.find(
    ...
    def second_time_run_case(
        self, year_value: int, month_value: int
    ...
    def run(self) -> bool | None:
        ...
        year = first_time_run_result.find(
            "select",
            {
                "id": "body_ddlYear",
            },
        ...
        runtime_options = first_time_run_result.find(
            "select",
            {
                "id": "body_ddlMoth",
            },
        ...
        next_options = [
            item for item in runtime_options.next_siblings if isinstance(item, Tag)
        ]
        for option in next_options:
            second_time_run_result = self.second_time_run_case(
                year_selected, int(option["value"])
            )
