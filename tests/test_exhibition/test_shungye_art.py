...
from helpers.headers_helper import UA, generate_cookies
...
def test_pased_shungye_art_html():
    ...
    notice = soup.find("a", {"id": "Notice"}).next_element.next_element.select(".indexnews1")
    now_ex = now + notice
    print()
    for ex in now_ex:
        title = ex.select_one("div.fig > h4").text.strip()
        source = "https://www.shungye-art.org/" + ex.find("a").attrs["href"].split("&")[0]
        fig = (
            "https://www.shungye-art.org/"
            + ex.select_one("a > div.info2").get("style").split("background: url(")[1].split(");")[0]
        )
