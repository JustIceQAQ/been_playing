...
def dbscan(df: pd.DataFrame):
    ...
    coords_rad = np.radians(coords_float)
    km_dist_matrix = haversine_distances(coords_rad, coords_rad) * EARTH_RADIUS_KM

    db_scan = DBSCAN(
        eps=EPS_KM,
        min_samples=MIN_SAMPLES,
        metric="precomputed",  # 告知 DBSCAN 使用我們已經計算好的距離矩陣
    ).fit(km_dist_matrix)
    ...
    output_file = DOCS_PATH / "been_play_dbscan_clusters_map.html"
    m.save(output_file)
